...
class ReviewRatingAdapter(private val recipeId: String="",private val reviewRatingList: MutableList<ReviewRating>):RecyclerView.Adapter<ReviewRatingViewHolder>() {
            ...
            if(ServiceBuilder.user!!._id == review.user._id) {
                    ...
                    if (writeReview.text.isNotEmpty()) {
                        ...
                        CoroutineScope(Dispatchers.IO).launch {
                            try {
                                ...
                                reviewRatingRepository.updateReviewRating(recipeId,ReviewRating(_id=review._id, review = writeReview.text.toString(), rating = selectedRating ))
                            } catch (ex: Exception){
                                print(ex)
                            }
                        }
                ...
                delete.setOnClickListener {
                    CoroutineScope(Dispatchers.IO).launch {
                        try {
                            val reviewRatingRepository=ReviewRatingRepository()
                            reviewRatingRepository.deleteReviewRating(recipeId,review._id)
                        } catch (ex: Exception){
                            print(ex)
                        }
                    }
